...
public class Homepage_PO {
	...
	By startLearning = By.cssSelector(".css-1jsa8x6");
	...
	By courseList = By.cssSelector(".css-1ty3oyt");
	...
	public void clickstartLearning() {
		driver.findElement(startLearning).click();
	}
	...
	public void openCourseByName(String expectedCourse) {

	    List<WebElement> courseCards = driver.findElements(
	        By.xpath("//div[contains(@class,'MuiGrid-item')]")
	    );

	    for (WebElement card : courseCards) {

	        String courseName = card.findElement(By.xpath(".//h6")).getText();

	        if (courseName.contains(expectedCourse)) {

	            WebElement courseImage =
	                card.findElement(By.xpath(".//img"));
	    ...
	    if (isNextPageAvailable()) {
	        clickNextPage();
	        openCourseByName(expectedCourse); // recursion
	    } else {
	        throw new RuntimeException("Course not found: " + expectedCourse);
	    }
	}
}
